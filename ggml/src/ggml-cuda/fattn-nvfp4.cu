...
#if defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && \
    defined(GGML_CUDA_NVFP4_FA_P1_KQ8_TC_PV_NOSPLIT)
__global__ void fattn_nvfp4_p1_kq8_tc_pv_nosplit_kernel(const fattn_nvfp4_mtp4_params params) {
#if defined(BLACKWELL_MMA_AVAILABLE)
    const int tid     = threadIdx.x;
    const int warp_id = tid / WARP_SIZE;
    const int lane    = tid & (WARP_SIZE - 1);
    ...
    if (params.ne_q_rows != 1 || q_head >= params.ne_q_heads || seq >= params.ne_seqs || kv_head >= params.ne_kv_heads) {
        return;
    }
    ...
    __shared__ float    smem_p;
    __shared__ float    smem_scale_old;
    __shared__ float    smem_pair_p0;
    __shared__ float    smem_pair_scale_old;
    __shared__ float    score_tile[tile_B::I];
    __shared__ float    pv_accum[FATTN_NVFP4_MAX_NCOL_TILE][WARP_SIZE][tile_C::ne];
...
#pragma unroll
    ...
    if (tid == 0) {
        ...
        smem_scale_old = 1.0f;
        smem_pair_p0 = 0.0f;
        smem_pair_scale_old = 1.0f;
    }

    for (int i = tid; i < FATTN_NVFP4_MAX_NCOL_TILE * WARP_SIZE * tile_C::ne;
            i += FATTN_NVFP4_TC_PV_WARPS * WARP_SIZE) {
        reinterpret_cast<float *>(pv_accum)[i] = 0.0f;
    }
    __syncthreads();

#pragma unroll 1
    for (int64_t kv_base = 0; kv_base < params.ne_kv_rows; kv_base += tile_B::I) {
        if (warp_id == 0) {
            tile_C kq_tile = {};
...
#pragma unroll
        for (int kv_i = 0; kv_i < tile_B::I; ++kv_i) {
            const int64_t kv_row = kv_base + kv_i;
            const bool valid_kv = kv_row < params.ne_kv_rows;

            if (tid == 0) {
                const float score = valid_kv ? score_tile[kv_i] : -INFINITY;
                if (isinf(score) && score < 0.0f) {
                    smem_p = 0.0f;
                    smem_scale_old = 1.0f;
                } else {
                    ...
                    smem_p = scale_new;
                    smem_scale_old = scale_old;
                }

                if ((kv_row & 1) == 0) {
                    smem_pair_p0 = smem_p;
                    smem_pair_scale_old = smem_scale_old;
                }
            }
            __syncthreads();

            if (valid_kv && ((kv_row & 1) != 0 || kv_row + 1 == params.ne_kv_rows) &&
                    warp_id < FATTN_NVFP4_TC_PV_WARPS) {
                const int64_t kv_row0 = (kv_row & 1) == 0 ? kv_row : kv_row - 1;
                const block_nvfp4 * v_ptr0 = params.V +
                    kv_head * params.v_stride_head +
                    seq     * params.v_stride_seq +
                    kv_row0 * params.v_stride_row;
                const block_nvfp4 * v_ptr1 = (kv_row & 1) != 0 ? params.V +
                    kv_head * params.v_stride_head +
                    seq     * params.v_stride_seq +
                    kv_row  * params.v_stride_row : nullptr;
...
#pragma unroll
                    for (int l = 0; l < tile_PV_A::ne; ++l) {
                        ...
                        const int col = ggml_cuda_fattn_nvfp4_pv_a_j(l, lane);
                        if (row == 0 && col == 0) {
                            A.x[l] = v_ptr1 != nullptr ?
                                __floats2half2_rn(smem_pair_p0 * smem_scale_old, smem_p) :
                                __floats2half2_rn(smem_pair_p0, 0.0f);
                        } else {
                            A.x[l] = __floats2half2_rn(0.0f, 0.0f);
                        }
                    }
...
#pragma unroll
                    for (int l = 0; l < tile_C::ne; ++l) {
                        const int row = ggml_cuda_fattn_nvfp4_pv_c_i(l, lane);
                        C.x[l] = row == 0 ?
                            pv_accum[tile_idx][lane][l] *
                                (v_ptr1 != nullptr ? smem_pair_scale_old * smem_scale_old : smem_pair_scale_old) :
                            0.0f;
                    }
...
#pragma unroll
            for (int l = 0; l < tile_C::ne; ++l) {
                const int row = ggml_cuda_fattn_nvfp4_pv_c_i(l, lane);
                const int col = col_base + ggml_cuda_fattn_nvfp4_pv_c_j(l, lane);
                if (row == 0 && col < params.v_head_dim && smem_rowsum != 0.0f) {
                    dst_ptr[col] = pv_accum[tile_idx][lane][l] / smem_rowsum;
                }
            }
        }
    }
...
#endif // defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && defined(GGML_CUDA_NVFP4_FA_P1_KQ8_TC_PV_NOSPLIT)
...
void ggml_cuda_flash_attn_ext_nvfp4_mtp4(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (params.ne_q_rows == 1) {
        const dim3 p1_grid((uint32_t) params.ne_q_heads, (uint32_t) params.ne_seqs, 1);
        const dim3 p1_block(FATTN_NVFP4_TC_PV_WARPS * WARP_SIZE, 1, 1);
        fattn_nvfp4_p1_kq8_tc_pv_nosplit_kernel<<<p1_grid, p1_block, 0, ctx.stream()>>>(params);
        CUDA_CHECK(cudaGetLastError());
        return;
    }
#endif // defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && defined(GGML_CUDA_NVFP4_FA_P1_KQ8_TC_PV_NOSPLIT)
