...
static void print_usage(const char * exe) {
    std::printf(
        "usage: %s [options]\n"
        "\n"
        "D=256 native-FP4 KQ probe at MTP tile height.\n"
        "\n"
        "options:\n"
...
__global__ void kq256_operand_kernel(
        const block_nvfp4 * q,
        const block_nvfp4 * k,
        size_t              block_mask,
        float *             out,
...
#pragma unroll
            for (int frag = 0; frag < nfrags; ++frag) {
                ...
                asm volatile(
                    ...
                    "%10, {0, 0}, %11, {0, 0};"
                    : "+f"(d0), "+f"(d1), "+f"(d2), "+f"(d3)
                    : "r"(q_a[frag][0]), "r"(q_a[frag][1]), "r"(q_a[frag][2]), "r"(q_a[frag][3]), "r"(bx0), "r"(bx1), "r"(q_scale[frag]), "r"(k_scale));
            }
        }
    }
...
static int run_benchmark(const bench_config & cfg, const cudaDeviceProp & prop, int cc) {
    ...
    if (cc < 1200 || cc >= 1300) {
        std::printf("kq256: skipped reason=requires Blackwell sm_120/sm_121 device\n");
        return 0;
    }

    int active_blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, kq256_operand_kernel, cfg.threads, 0));
    ...
    std::printf("kq256 occupancy: active_blocks_per_sm=%d active_warps_per_sm=%d occupancy=%.1f%% shared=0.000 KiB\n",
    ...
    kq256_operand_kernel<<<cfg.blocks, cfg.threads>>>(q, k, n_blocks - 1, out, 10);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());

    CUDA_CHECK(cudaEventRecord(start));
    kq256_operand_kernel<<<cfg.blocks, cfg.threads>>>(q, k, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaEventRecord(stop));
    const float kq_ms = time_events(start, stop);

    const double seconds = kq_ms / 1000.0;
    ...
    std::printf("kq256: %.3f KQ-TOPS  %.3f useful-mtp-KQ-TOPS  %.3f GB/s-K-compact-read  %.6f GB-Q-compact-once  blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
                ...
                kq_ops / seconds / 1.0e12 * useful_mtp_fraction(cfg),
