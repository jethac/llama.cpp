...
static void print_usage(const char * exe) {
    std::printf(
        "usage: %s [options]\n"
        "\n"
        "Phase-separated FP4-KQ + real compact-NVFP4-to-FP16 mixed-PV handoff probe.\n"
        "\n"
        "options:\n"
        ...
        "  --groups N     mixed-PV groups per key tile (1,2,4,8; default: 1)\n"
...
__global__ void realhandoff_kernel(
        const block_nvfp4 * v,
        size_t              block_mask,
        int                 groups,
        float *             out,
...
int main(int argc, char ** argv) {
    ...
    if (cc < 1200 || cc >= 1300) {
        std::printf("realhandoff: skipped reason=requires Blackwell sm_120/sm_121 device\n");
        return 0;
    }

    int active_blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, realhandoff_kernel, cfg.threads, 0));
    ...
    std::printf("realhandoff occupancy: active_blocks_per_sm=%d active_warps_per_sm=%d occupancy=%.1f%% shared=0.000 KiB\n",
    ...
    realhandoff_kernel<<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, cfg.groups, out, 10);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    realhandoff_kernel<<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, cfg.groups, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    ...
    std::printf("realhandoff: %.3f measured-total-TOPS  %.3f useful-mtp-measured-total-TOPS  %.3f KQ-issue-TOPS  %.3f measured-mixedPV-TOPS  %.3f projected-fullPV-TOPS-at-same-time  %.3f GB/s-real-compact-read  pv_group_fraction=%.3f groups=%d blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
