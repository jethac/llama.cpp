...
#if defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_P1_KQ8_SCALAR_PV)
__global__ void fattn_nvfp4_p1_kq8_scalar_pv_kernel(const fattn_nvfp4_mtp4_params params) {
#if defined(BLACKWELL_MMA_AVAILABLE)
    const int lane = threadIdx.x & (WARP_SIZE - 1);
    ...
    static_assert(FATTN_NVFP4_MAX_HEAD_DIM % WARP_SIZE == 0, "bad p1 scalar PV head dimension");
...
#pragma unroll
    ...
    float kq_max = -3.402823466e+38F;
    float rowsum = 0.0f;
    float pv[FATTN_NVFP4_MAX_HEAD_DIM / WARP_SIZE];

#pragma unroll
    for (int i = 0; i < FATTN_NVFP4_MAX_HEAD_DIM / WARP_SIZE; ++i) {
        pv[i] = 0.0f;
    }
...
#pragma unroll 1
    for (int64_t kv_base = 0; kv_base < params.ne_kv_rows; kv_base += tile_B::I) {
        ...
        for (int frag = 0; frag < nfrag; ++frag) {
            ...
            if (lane == 0) {
                kq_a_scale[0] = q_tile_scale[frag];
            }
...
#pragma unroll
            ...
            scores[kv_i] = __shfl_sync(0xffffffff, local_score, 0);
        }
...
#pragma unroll
        for (int kv_i = 0; kv_i < tile_B::I; ++kv_i) {
            ...
            const float score = scores[kv_i] * params.scale + mask;
            const float kq_max_new = fmaxf(kq_max, score);
            const float scale_old = rowsum == 0.0f ? 0.0f : expf(kq_max - kq_max_new);
            const float scale_new = score - kq_max_new >= SOFTMAX_FTZ_THRESHOLD ? expf(score - kq_max_new) : 0.0f;
...
#pragma unroll
            for (int col_slot = 0; col_slot < FATTN_NVFP4_MAX_HEAD_DIM / WARP_SIZE; ++col_slot) {
                const int col = lane + col_slot * WARP_SIZE;
                if (col < params.v_head_dim) {
                    pv[col_slot] = pv[col_slot] * scale_old + scale_new * ggml_cuda_fattn_nvfp4_dequant_row_value(v_ptr, col);
                }
            }
...
#pragma unroll
    for (int col_slot = 0; col_slot < FATTN_NVFP4_MAX_HEAD_DIM / WARP_SIZE; ++col_slot) {
        const int col = lane + col_slot * WARP_SIZE;
        if (col < params.v_head_dim) {
            dst_ptr[col] = rowsum == 0.0f ? 0.0f : pv[col_slot] / rowsum;
        }
    }
#else
    GGML_UNUSED(params);
...
#endif // defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_P1_KQ8_SCALAR_PV)
...
void ggml_cuda_flash_attn_ext_nvfp4_mtp4(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (params.ne_q_rows == 1) {
        const dim3 blocks_num((uint32_t) params.ne_q_heads, (uint32_t) params.ne_seqs, 1);
        const dim3 block_dim(WARP_SIZE, 1, 1);
        fattn_nvfp4_p1_kq8_scalar_pv_kernel<<<blocks_num, block_dim, 0, ctx.stream()>>>(params);
        CUDA_CHECK(cudaGetLastError());
        return;
    }
#endif // defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_P1_KQ8_SCALAR_PV)
