...
static void print_usage(const char * exe) {
    std::printf(
        "usage: %s [options]\n"
        "\n"
        "Register-fragment queue probe for phase-split FP4-KQ + mixed-PV.\n"
        "\n"
        "options:\n"
        ...
        "  --groups N     register-queued mixed-PV groups per key tile (1,2,4,8; default: 1)\n"
...
template <int GROUPS>
__global__ void regqueue_kernel(
        const block_nvfp4 * v,
        size_t              block_mask,
        float *             out,
        uint64_t            iters) {
...
#pragma unroll
        for (int tile = 0; tile < 64; ++tile) {
            ...
            uint32_t word0[GROUPS];
            uint32_t word1[GROUPS];
            uint8_t d0[GROUPS];
            uint8_t d1[GROUPS];

#pragma unroll
            for (int group = 0; group < GROUPS; ++group) {
                word0[group] = nvfp4_group_word(blk0, group);
                word1[group] = nvfp4_group_word(blk1, group);
                d0[group] = blk0.d[group / 2];
                d1[group] = blk1.d[group / 2];
            }
...
#pragma unroll
            for (int group = 0; group < GROUPS; ++group) {
                const float fd0 = ue4m3_to_fp32(d0[group]);
                const float fd1 = ue4m3_to_fp32(d1[group]);

#pragma unroll
                for (int kk = 0; kk < 4; ++kk) {
                    const uint8_t q0 = (uint8_t) (word0[group] >> (8 * kk));
                    const uint8_t q1 = (uint8_t) (word1[group] >> (8 * kk));
                    const uint32_t pv_bx0 = half2_bits(dequant_nvfp4_byte_to_half2(q0, fd0));
                    const uint32_t pv_bx1 = half2_bits(dequant_nvfp4_byte_to_half2(q1, fd1));
                    asm volatile(
...
static int run_benchmark(const bench_config & cfg, const cudaDeviceProp & prop, int cc) {
    std::printf("config:               blocks=%d threads=%d mtp_rows=%d useful_m16=%.1f%% groups=%d iters=%" PRIu64 "\n",
                cfg.blocks, cfg.threads, cfg.mtp_rows, 100.0 * useful_mtp_fraction(cfg), GROUPS, cfg.iters);

    if (cc < 1200 || cc >= 1300) {
        std::printf("regqueue: skipped reason=requires Blackwell sm_120/sm_121 device\n");
        return 0;
    }

    int active_blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, regqueue_kernel<GROUPS>, cfg.threads, 0));
    ...
    std::printf("regqueue occupancy: active_blocks_per_sm=%d active_warps_per_sm=%d occupancy=%.1f%% shared=0.000 KiB\n",
    ...
    regqueue_kernel<GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, 10);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    regqueue_kernel<GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    ...
    const double measured_pv_ops = (double) n_warps * (double) cfg.iters * (double) kq_tiles * (double) GROUPS * 4.0 * 2.0 * 16.0 * 8.0 * 16.0;
    ...
    const double reg_bytes = (double) n_warps * (double) cfg.iters * (double) kq_tiles * 32.0 * (double) GROUPS * 10.0 / 1.0e9;

    std::printf("regqueue: %.3f measured-total-TOPS  %.3f useful-mtp-measured-total-TOPS  %.3f KQ-issue-TOPS  %.3f measured-mixedPV-TOPS  %.3f projected-fullPV-TOPS-at-same-time  %.3f GB/s-compact-read-once  %.3f GB/s-register-frag-bytes  pv_group_fraction=%.3f groups=%d blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
                ...
                reg_bytes / seconds,
                (double) GROUPS / (double) full_groups,
                GROUPS,
                n_blocks,
                n_warps,
                cfg.iters,
...
int main(int argc, char ** argv) {
    ...
    switch (cfg.groups) {
        ...
        case 8: return run_benchmark<8>(cfg, prop, cc);
        default: return 1;
    }
}
