...
static void print_usage(const char * exe) {
    std::printf(
        "usage: %s [options]\n"
        "\n"
        "Tiny compact-fragment queue probe for phase-split FP4-KQ + mixed-PV.\n"
        "\n"
        "options:\n"
        ...
        "  --groups N        queued mixed-PV groups per key tile (1,2,4,8; default: 1)\n"
        "  --queue-tiles N   KQ tiles captured before PV consume (1,2,4,8; default: 1)\n"
...
static bench_config parse_args(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (std::strcmp(arg, "--queue-tiles") == 0) {
            cfg.queue_tiles = (int) parse_u64(require_value(arg), arg);
    ...
    if (cfg.queue_tiles != 1 && cfg.queue_tiles != 2 && cfg.queue_tiles != 4 && cfg.queue_tiles != 8) {
        std::fprintf(stderr, "invalid --queue-tiles value: %d (expected 1, 2, 4, or 8)\n", cfg.queue_tiles);
        std::exit(1);
    }
...
static __device__ __forceinline__ uint32_t nvfp4_group_word(const block_nvfp4 & blk, int group) {
...
__global__ void fragqueue_kernel(
        const block_nvfp4 * v,
        size_t              block_mask,
        int                 groups,
        int                 queue_tiles,
    ...
    extern __shared__ queued_frag queue[];
    ...
    for (uint64_t i = 0; i < iters; ++i) {
        for (int tile_base = 0; tile_base < 64; tile_base += queue_tiles) {
            for (int qt = 0; qt < queue_tiles; ++qt) {
                const int tile = tile_base + qt;
                ...
                for (int group = 0; group < groups; ++group) {
                    queued_frag & frag = queue[((threadIdx.x * queue_tiles + qt) * groups) + group];
                    frag.word0 = nvfp4_group_word(blk0, group);
                    frag.word1 = nvfp4_group_word(blk1, group);
                    frag.d0 = blk0.d[group / 2];
                    frag.d1 = blk1.d[group / 2];
                    frag.pad = 0;
                }
            ...
            for (int qt = 0; qt < queue_tiles; ++qt) {
                for (int group = 0; group < groups; ++group) {
                    const queued_frag frag = queue[((threadIdx.x * queue_tiles + qt) * groups) + group];
                    const float d0 = ue4m3_to_fp32(frag.d0);
                    const float d1 = ue4m3_to_fp32(frag.d1);

#pragma unroll
                    for (int kk = 0; kk < 4; ++kk) {
                        const uint8_t q0 = (uint8_t) (frag.word0 >> (8 * kk));
                        const uint8_t q1 = (uint8_t) (frag.word1 >> (8 * kk));
                    ...
                    const float mix = (float) ((tile_base + qt) * 8 + group + 1) * 0.000001f / (row_l0 + row_l1 + 0.000001f);
                    out0 += pv0 * mix;
                    out1 += pv1 * mix;
                    out2 += pv2 * mix;
                    out3 += pv3 * mix;
...
int main(int argc, char ** argv) {
    ...
    const size_t shared_bytes = (size_t) cfg.threads * (size_t) cfg.queue_tiles * (size_t) cfg.groups * sizeof(queued_frag);
    ...
    std::printf("config:               blocks=%d threads=%d mtp_rows=%d useful_m16=%.1f%% groups=%d queue_tiles=%d iters=%" PRIu64 "\n",
                cfg.blocks, cfg.threads, cfg.mtp_rows, 100.0 * useful_mtp_fraction(cfg), cfg.groups, cfg.queue_tiles, cfg.iters);

    if (cc < 1200 || cc >= 1300) {
        std::printf("fragqueue: skipped reason=requires Blackwell sm_120/sm_121 device\n");
        return 0;
    }

    if (shared_bytes > (size_t) prop.sharedMemPerBlockOptin) {
        std::printf("fragqueue: skipped reason=shared_bytes=%zu exceeds optin limit=%zu\n",
                    shared_bytes, (size_t) prop.sharedMemPerBlockOptin);
        return 0;
    }

    if (shared_bytes > (size_t) prop.sharedMemPerBlock) {
        CUDA_CHECK(cudaFuncSetAttribute(fragqueue_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int) shared_bytes));
    }

    int active_blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, fragqueue_kernel, cfg.threads, shared_bytes));
    ...
    std::printf("fragqueue occupancy: active_blocks_per_sm=%d active_warps_per_sm=%d occupancy=%.1f%% shared=%.3f KiB\n",
                active_blocks_per_sm, active_warps_per_sm, 100.0 * occupancy, (double) shared_bytes / 1024.0);

    if (active_blocks_per_sm == 0) {
        std::printf("fragqueue: skipped reason=zero active blocks for requested shared memory\n");
        return 0;
    }
    ...
    fragqueue_kernel<<<cfg.blocks, cfg.threads, shared_bytes>>>(v, n_blocks - 1, cfg.groups, cfg.queue_tiles, out, 10);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    fragqueue_kernel<<<cfg.blocks, cfg.threads, shared_bytes>>>(v, n_blocks - 1, cfg.groups, cfg.queue_tiles, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    ...
    const double compact_read_gb = (double) n_warps * (double) cfg.iters * (double) kq_tiles * 64.0 * (double) sizeof(block_nvfp4) / 1.0e9;
    const double queue_write_gb = (double) n_threads * (double) cfg.iters * (double) kq_tiles * (double) cfg.groups * (double) sizeof(queued_frag) / 1.0e9;

    std::printf("fragqueue: %.3f measured-total-TOPS  %.3f useful-mtp-measured-total-TOPS  %.3f KQ-issue-TOPS  %.3f measured-mixedPV-TOPS  %.3f projected-fullPV-TOPS-at-same-time  %.3f GB/s-compact-read-once  %.3f GB/s-fragqueue-write  pv_group_fraction=%.3f groups=%d queue_tiles=%d shared=%.3fKiB blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
                ...
                compact_read_gb / seconds,
                queue_write_gb / seconds,
