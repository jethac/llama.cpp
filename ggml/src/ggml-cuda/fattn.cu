...
#define FATTN_VEC_CASE(DKQ, DV, type_K, type_V)                                                                 \
    {                                                                                                            \
        const bool type_K_okay = K->type == (type_K) || (K->type == GGML_TYPE_F32 && (type_K) == GGML_TYPE_F16); \
        const bool type_V_okay = V->type == (type_V) || (V->type == GGML_TYPE_F32 && (type_V) == GGML_TYPE_F16); \
        if (Q->ne[0] == (DKQ) && V->ne[0] == (DV) && type_K_okay && type_V_okay) {                               \
            ggml_cuda_flash_attn_ext_vec_case<DKQ, DV, type_K, type_V>(ctx, dst);                                \
            return;                                                                                              \
        }                                                                                                        \
    }                                                                                                            \

#define FATTN_VEC_CASES_ALL_D(type_K, type_V) \
    FATTN_VEC_CASE( 64,  64, type_K, type_V) \
    FATTN_VEC_CASE(128, 128, type_K, type_V) \
    FATTN_VEC_CASE(256, 256, type_K, type_V) \
...
static void ggml_cuda_flash_attn_ext_vec(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
...
#if defined(GGML_CUDA_NVFP4_KV_EXEC_P1_NATIVE_KQ) && defined(GGML_CUDA_NVFP4_FA_P1_KX_PROB_LOGICAL_V_FP16_PV)
    if (ggml_cuda_flash_attn_ext_nvfp4_p1_kx_logical_v_fp16_pv(ctx, dst)) {
        ggml_cuda_nvfp4_fattn_trace(ctx, "nvfp4-kx-prob-logical-v-fp16-pv", dst);
        return;
    }
#endif
    ...
    if (ggml_cuda_flash_attn_ext_nvfp4_p1_kx_vx_native_pv(ctx, dst)) {
        ggml_cuda_nvfp4_fattn_trace(ctx, "nvfp4-kx-vx-native-pv", dst);
        return;
    }
#endif
#if defined(GGML_CUDA_NVFP4_KV_EXEC_P1_NATIVE_KQ)
...
#if defined(GGML_CUDA_NVFP4_FA_P1_KQ8_TC_PV)
...
size_t ggml_cuda_flash_attn_ext_get_alloc_size(int device, const ggml_tensor * dst) {
    ...
    if (kernel == BEST_FATTN_KERNEL_NVFP4_MTP4) {
        return ggml_cuda_flash_attn_ext_nvfp4_mtp4_get_alloc_size(dst);
    }
