...
__global__ void predecode_v_bready_kernel(const block_nvfp4 * src, uint32_t * dst, size_t n_blocks, uint64_t repeats) {
    constexpr int groups_per_frag = QK_NVFP4 / 8;
    constexpr int entries_per_block = groups_per_frag * 4;
    ...
    for (uint64_t r = 0; r < repeats; ++r) {
        for (size_t b = tid; b < n_blocks; b += stride) {
            const block_nvfp4 blk = src[b];
            const uint32_t * qs = reinterpret_cast<const uint32_t *>(blk.qs);
            uint32_t * out = dst + b * entries_per_block;

#pragma unroll
            for (int group = 0; group < groups_per_frag; ++group) {
                const uint32_t word = qs[group];
                const float d = ue4m3_to_fp32(blk.d[group / 2]);

#pragma unroll
                for (int kk = 0; kk < 4; ++kk) {
                    const uint8_t q = (uint8_t) (word >> (8 * kk));
                    out[group * 4 + kk] = half2_bits(dequant_nvfp4_byte_to_half2(q, d));
                }
            }
        }
    }
...
__global__ void fill_nvfp4_bready_lut_kernel(uint32_t * lut) {
...
__global__ void mixedpv_predecodedb_kernel(
        const block_nvfp4 * q,
        const block_nvfp4 * k,
        const uint32_t *    v_bready,
        size_t              block_mask,
        float *             out,
        uint64_t            iters) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1200 && __CUDA_ARCH__ < 1300
    ...
    constexpr int k_tiles = 64;
    constexpr int groups_per_frag = QK_NVFP4 / 8;
    constexpr int bready_entries_per_block = groups_per_frag * 4;
...
#pragma unroll
            for (int frag = 0; frag < nfrags; ++frag) {
                const size_t idx0 = ((((row_base + (size_t) (2 * lane + 0)) * (size_t) nfrags) + (size_t) frag) & block_mask) *
                                    (size_t) bready_entries_per_block;
                const size_t idx1 = ((((row_base + (size_t) (2 * lane + 1)) * (size_t) nfrags) + (size_t) frag) & block_mask) *
                                    (size_t) bready_entries_per_block;
...
#pragma unroll
                    for (int kk = 0; kk < 4; ++kk) {
                        const uint32_t bx0 = v_bready[idx0 + (size_t) group * 4u + (size_t) kk];
                        const uint32_t bx1 = v_bready[idx1 + (size_t) group * 4u + (size_t) kk];
                        pv_mma_accumulate(pax0, pax1, pax2, pax3, bx0, bx1, pv0, pv1, pv2, pv3);
                    }
    ...
    (void) q; (void) k; (void) v_bready; (void) block_mask; (void) out; (void) iters;
#endif
}

__global__ void mixedpv_lutb_kernel(
        const block_nvfp4 * q,
        const block_nvfp4 * k,
        const block_nvfp4 * v,
...
#pragma unroll
                for (int group = 0; group < groups_per_frag; ++group) {
                    const uint32_t word0 = v0[group];
                    const uint32_t word1 = v1[group];
                    const uint32_t scale0 = (uint32_t) blk0.d[group / 2] << 8;
                    const uint32_t scale1 = (uint32_t) blk1.d[group / 2] << 8;

#pragma unroll
                    for (int kk = 0; kk < 4; ++kk) {
                        const uint8_t q0b = (uint8_t) (word0 >> (8 * kk));
                        const uint8_t q1b = (uint8_t) (word1 >> (8 * kk));
                        const uint32_t bx0 = v_lut[scale0 | (uint32_t) q0b];
                        const uint32_t bx1 = v_lut[scale1 | (uint32_t) q1b];
                        pv_mma_accumulate(pax0, pax1, pax2, pax3, bx0, bx1, pv0, pv1, pv2, pv3);
                    }
    ...
    (void) q; (void) k; (void) v; (void) v_lut; (void) block_mask; (void) out; (void) iters;
#endif
}

__global__ void mixedpv_lutb_reuse4_kernel(
        const block_nvfp4 * q,
        const block_nvfp4 * k,
        const block_nvfp4 * v,
        const uint32_t *    v_lut,
...
#pragma unroll
            ...
            const float p_base = 1.0f / (row_l0 + 1.0f);
            const uint32_t pax00 = half2_bits(__floats2half2_rn(p_base, p_base * 0.9375f));
            const uint32_t pax01 = half2_bits(__floats2half2_rn(p_base * 0.8750f, p_base * 0.8125f));
            const uint32_t pax02 = half2_bits(__floats2half2_rn(p_base * 0.7500f, p_base * 0.6875f));
            const uint32_t pax03 = half2_bits(__floats2half2_rn(p_base * 0.6250f, p_base * 0.5625f));
            const uint32_t pax10 = half2_bits(__floats2half2_rn(p_base * 0.9688f, p_base * 0.9062f));
            const uint32_t pax11 = half2_bits(__floats2half2_rn(p_base * 0.8438f, p_base * 0.7812f));
            const uint32_t pax12 = half2_bits(__floats2half2_rn(p_base * 0.7188f, p_base * 0.6562f));
            const uint32_t pax13 = half2_bits(__floats2half2_rn(p_base * 0.5938f, p_base * 0.5312f));
            const uint32_t pax20 = half2_bits(__floats2half2_rn(p_base * 0.9844f, p_base * 0.9219f));
            const uint32_t pax21 = half2_bits(__floats2half2_rn(p_base * 0.8594f, p_base * 0.7969f));
            const uint32_t pax22 = half2_bits(__floats2half2_rn(p_base * 0.7344f, p_base * 0.6719f));
            const uint32_t pax23 = half2_bits(__floats2half2_rn(p_base * 0.6094f, p_base * 0.5469f));
            const uint32_t pax30 = half2_bits(__floats2half2_rn(p_base * 0.9531f, p_base * 0.8906f));
            const uint32_t pax31 = half2_bits(__floats2half2_rn(p_base * 0.8281f, p_base * 0.7656f));
            const uint32_t pax32 = half2_bits(__floats2half2_rn(p_base * 0.7031f, p_base * 0.6406f));
            const uint32_t pax33 = half2_bits(__floats2half2_rn(p_base * 0.5781f, p_base * 0.5156f));
...
#pragma unroll
                    for (int kk = 0; kk < 4; ++kk) {
                        ...
                        const uint32_t bx0 = v_lut[scale0 | (uint32_t) q0b];
                        const uint32_t bx1 = v_lut[scale1 | (uint32_t) q1b];
                        pv_mma_accumulate(pax00, pax01, pax02, pax03, bx0, bx1, pv00, pv01, pv02, pv03);
                        pv_mma_accumulate(pax10, pax11, pax12, pax13, bx0, bx1, pv10, pv11, pv12, pv13);
                        pv_mma_accumulate(pax20, pax21, pax22, pax23, bx0, bx1, pv20, pv21, pv22, pv23);
                        pv_mma_accumulate(pax30, pax31, pax32, pax33, bx0, bx1, pv30, pv31, pv32, pv33);
                    }

                    sink += (pv00 + pv01 + pv02 + pv03 + pv10 + pv11 + pv12 + pv13 +
                             pv20 + pv21 + pv22 + pv23 + pv30 + pv31 + pv32 + pv33) *
                            (float) (frag * groups_per_frag + group + 1) * 0.000000025f;
                }
            }
        }
    }
    ...
    if (lane == 0) {
        out[warp] = kq0 + kq1 + kq2 + kq3 + pv00 + pv01 + pv02 + pv03 + pv10 + pv11 + pv12 + pv13 +
                    pv20 + pv21 + pv22 + pv23 + pv30 + pv31 + pv32 + pv33 + row_m0 + row_l0 + sink;
    }
#else
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        out[0] = 0.0f;
...
__global__ void mixedpv_lutb_mtp4_kernel(
        const block_nvfp4 * q,
        const block_nvfp4 * k,
        const block_nvfp4 * v,
        const uint32_t *    v_lut,
    ...
    constexpr int k_tiles = 64;
    constexpr int groups_per_frag = QK_NVFP4 / 8;
    constexpr int mtp_rows = 4;

    const int lane = threadIdx.x & 31;
    const int warp = ((int) blockIdx.x * blockDim.x + threadIdx.x) >> 5;

    int q_a[mtp_rows][nfrags][4];
    uint32_t q_scale[mtp_rows][nfrags];

#pragma unroll
    for (int row = 0; row < mtp_rows; ++row) {
#pragma unroll
        for (int frag = 0; frag < nfrags; ++frag) {
            const size_t q_idx =
                (((size_t) warp * (size_t) mtp_rows + (size_t) row) * (size_t) nfrags * 32u +
                 (size_t) frag * 32u + (size_t) lane) & block_mask;
            const block_nvfp4 q_blk = q[q_idx];
            const uint32_t * q_qs = reinterpret_cast<const uint32_t *>(q_blk.qs);
            q_a[row][frag][0] = (int) q_qs[(lane + 0) & 7];
            q_a[row][frag][1] = (int) q_qs[(lane + 1) & 7];
            q_a[row][frag][2] = (int) q_qs[(lane + 2) & 7];
            q_a[row][frag][3] = (int) q_qs[(lane + 3) & 7];
            q_scale[row][frag] = *reinterpret_cast<const uint32_t *>(q_blk.d);
        }
    }

    float kq[mtp_rows][4] = {};
    float pv[mtp_rows][4] = {};
    float row_m[mtp_rows] = { -64.0f, -64.0f, -64.0f, -64.0f };
    float row_l[mtp_rows] = {};
    float sink = 0.0f;
...
#pragma unroll
                for (int row = 0; row < mtp_rows; ++row) {
                    kq_mma_accumulate(
                        q_a[row][frag][0], q_a[row][frag][1], q_a[row][frag][2], q_a[row][frag][3],
                        bx0, bx1, q_scale[row][frag], k_scale,
                        kq[row][0], kq[row][1], kq[row][2], kq[row][3]);
                }
            }

            uint32_t pax[mtp_rows][4];
            const int q_row = lane & 15;
            const int base_pos = (int) (i & 1023u);

#pragma unroll
            for (int row = 0; row < mtp_rows; ++row) {
                const int logical_q = base_pos + q_row + row;
                const bool causal = tile <= logical_q;
                const bool swa = tile + 1024 >= logical_q;
                const float score = (causal && swa) ? (kq[row][0] + kq[row][1] + kq[row][2] + kq[row][3]) * 0.000244140625f : -64.0f;
                const float next_m = fmaxf(row_m[row], score);
                const float alpha = row_l[row] == 0.0f ? 0.0f : exp2f(row_m[row] - next_m);
                row_l[row] = row_l[row] * alpha + exp2f(score - next_m);
                row_m[row] = next_m;

                const float p_base = 1.0f / (row_l[row] + 1.0f);
                pax[row][0] = half2_bits(__floats2half2_rn(p_base, p_base * 0.9375f));
                pax[row][1] = half2_bits(__floats2half2_rn(p_base * 0.8750f, p_base * 0.8125f));
                pax[row][2] = half2_bits(__floats2half2_rn(p_base * 0.7500f, p_base * 0.6875f));
                pax[row][3] = half2_bits(__floats2half2_rn(p_base * 0.6250f, p_base * 0.5625f));
            }
...
#pragma unroll
                        for (int row = 0; row < mtp_rows; ++row) {
                            pv_mma_accumulate(
                                pax[row][0], pax[row][1], pax[row][2], pax[row][3],
                                bx0, bx1,
                                pv[row][0], pv[row][1], pv[row][2], pv[row][3]);
                        }
                    }

#pragma unroll
                    for (int row = 0; row < mtp_rows; ++row) {
                        sink += (pv[row][0] + pv[row][1] + pv[row][2] + pv[row][3]) *
                                (float) ((row + 1) * (frag * groups_per_frag + group + 1)) * 0.000000025f;
                    }
                }
            }
        }
...
#pragma unroll
        for (int row = 0; row < mtp_rows; ++row) {
            sum += row_m[row] + row_l[row];
#pragma unroll
            for (int j = 0; j < 4; ++j) {
                sum += kq[row][j] + pv[row][j];
            }
        }
        out[warp] = sum;
    }
...
static int run_predecodedb_benchmark(
        const bench_config & cfg,
        const cudaDeviceProp & prop,
        const block_nvfp4 * q,
        const block_nvfp4 * k,
        const uint32_t * v_bready,
        float predecode_ms,
        uint64_t predecode_repeats,
        size_t n_blocks,
        size_t n_warps,
        float * out);

static int run_lutb_benchmark(
        const bench_config & cfg,
        const cudaDeviceProp & prop,
        const block_nvfp4 * q,
        const block_nvfp4 * k,
...
static int run_lutb_reuse4_benchmark(
        const bench_config & cfg,
        const cudaDeviceProp & prop,
        const block_nvfp4 * q,
        const block_nvfp4 * k,
...
static int run_lutb_mtp4_benchmark(
        const bench_config & cfg,
        const cudaDeviceProp & prop,
        const block_nvfp4 * q,
        const block_nvfp4 * k,
...
static int run_benchmark(const bench_config & cfg, const cudaDeviceProp & prop, int cc) {
    ...
    fill_nvfp4_bready_lut_kernel<<<256, 256>>>(v_lut);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    constexpr uint64_t predecode_repeats = 4;
    predecode_v_bready_kernel<<<cfg.blocks, cfg.threads>>>(v, v_bready, n_blocks, 1);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaEventRecord(start));
    predecode_v_bready_kernel<<<cfg.blocks, cfg.threads>>>(v, v_bready, n_blocks, predecode_repeats);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaEventRecord(stop));
    const float predecode_ms = time_events(start, stop);
    const double predecode_seconds = predecode_ms / 1000.0;
    const double predecode_in_gb = (double) n_blocks * (double) sizeof(block_nvfp4) * (double) predecode_repeats / 1.0e9;
    const double predecode_out_gb = (double) n_blocks * (double) bready_entries_per_block * (double) sizeof(uint32_t) *
                                    (double) predecode_repeats / 1.0e9;
    std::printf("v_predecode_bready: %.3f GB/s-compact-read  %.3f GB/s-bready-write  %.3f GB/s-total  %.3f GB-bready-once  blocks=%zu repeats=%" PRIu64 " time=%.3f ms\n",
                predecode_in_gb / predecode_seconds,
                predecode_out_gb / predecode_seconds,
                (predecode_in_gb + predecode_out_gb) / predecode_seconds,
                (double) n_blocks * (double) bready_entries_per_block * (double) sizeof(uint32_t) / 1.0e9,
                n_blocks,
                predecode_repeats,
                predecode_ms);
    ...
    run_lutb_reuse4_benchmark(cfg, prop, q, k, v, v_lut, n_blocks, n_warps, out);
    run_lutb_mtp4_benchmark(cfg, prop, q, k, v, v_lut, n_blocks, n_warps, out);
...
static int run_predecodedb_benchmark(
        ...
        const block_nvfp4 * k,
        const uint32_t * v_bready,
        const float predecode_ms,
        const uint64_t predecode_repeats,
    ...
    const char * label = "combined256_predecodedb";
    const int active = print_occupancy(cfg, prop, label, (const void *) mixedpv_predecodedb_kernel);
    if (active == 0) {
        std::printf("%s: skipped reason=kernel has zero active blocks for threads=%d\n", label, cfg.threads);
        return 0;
    }

    mixedpv_predecodedb_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v_bready, n_blocks - 1, out, 2);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    CUDA_CHECK(cudaEventCreate(&stop));
    CUDA_CHECK(cudaEventRecord(start));
    mixedpv_predecodedb_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v_bready, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaEventRecord(stop));
    const float ms = time_events(start, stop);

    const double seconds = ms / 1000.0;
    const double predecode_seconds_once = predecode_ms / 1000.0 / (double) predecode_repeats;
    const double effective_seconds = seconds + predecode_seconds_once;
    const double kq_ops = (double) n_warps * (double) cfg.iters * (double) k_tiles * (double) nfrags * 2.0 * 16.0 * 8.0 * 64.0;
    const double pv_ops = (double) n_warps * (double) cfg.iters * (double) k_tiles * (double) nfrags *
                          (double) groups_per_frag * 4.0 * 2.0 * 16.0 * 8.0 * 16.0;
    const double k_compact_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * (double) nfrags * 32.0 * (double) sizeof(block_nvfp4) / 1.0e9;
    const double v_bready_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * 64.0 *
                              (double) nfrags * (double) bready_entries_per_block * (double) sizeof(uint32_t) / 1.0e9;

    std::printf("%s: %.3f modeled-total-TOPS  %.3f useful-mtp-modeled-total-TOPS  %.3f effective-with-predecode-TOPS  %.3f KQ-issue-TOPS  %.3f mixedPV-issue-TOPS  %.3f GB/s-K-compact-read  %.3f GB/s-V-bready-read  predecode_once_ms=%.3f blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
                label,
                (kq_ops + pv_ops) / seconds / 1.0e12,
                (kq_ops + pv_ops) / seconds / 1.0e12 * useful_mtp_fraction(cfg),
                (kq_ops + pv_ops) / effective_seconds / 1.0e12,
                kq_ops / seconds / 1.0e12,
                pv_ops / seconds / 1.0e12,
                k_compact_gb / seconds,
                v_bready_gb / seconds,
                predecode_seconds_once * 1000.0,
                n_blocks,
                n_warps,
                cfg.iters,
                ms);
...
static int run_lutb_benchmark(
    ...
    const char * label = "combined256_lutb";
    const int active = print_occupancy(cfg, prop, label, (const void *) mixedpv_lutb_kernel);
    if (active == 0) {
        std::printf("%s: skipped reason=kernel has zero active blocks for threads=%d\n", label, cfg.threads);
        return 0;
    }

    mixedpv_lutb_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v, v_lut, n_blocks - 1, out, 2);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    CUDA_CHECK(cudaEventCreate(&stop));
    CUDA_CHECK(cudaEventRecord(start));
    mixedpv_lutb_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v, v_lut, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaEventRecord(stop));
    const float ms = time_events(start, stop);
    ...
    const double v_lut_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * 64.0 *
                            (double) nfrags * (double) groups_per_frag * 4.0 * 2.0 * (double) sizeof(uint32_t) / 1.0e9;

    std::printf("%s: %.3f modeled-total-TOPS  %.3f useful-mtp-modeled-total-TOPS  %.3f KQ-issue-TOPS  %.3f mixedPV-issue-TOPS  %.3f GB/s-K-compact-read  %.3f GB/s-V-compact-read  %.3f GB/s-V-lut-read  lut_bytes=%zu blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
                label,
                (kq_ops + pv_ops) / seconds / 1.0e12,
                (kq_ops + pv_ops) / seconds / 1.0e12 * useful_mtp_fraction(cfg),
                kq_ops / seconds / 1.0e12,
...
static int run_lutb_reuse4_benchmark(
    ...
    constexpr int k_tiles = 64;
    constexpr int groups_per_frag = QK_NVFP4 / 8;
    constexpr int reuse_factor = 4;

    const char * label = "combined256_lutb_reuse4";
    const int active = print_occupancy(cfg, prop, label, (const void *) mixedpv_lutb_reuse4_kernel);
    if (active == 0) {
        std::printf("%s: skipped reason=kernel has zero active blocks for threads=%d\n", label, cfg.threads);
        return 0;
    }

    mixedpv_lutb_reuse4_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v, v_lut, n_blocks - 1, out, 2);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    CUDA_CHECK(cudaEventCreate(&stop));
    CUDA_CHECK(cudaEventRecord(start));
    mixedpv_lutb_reuse4_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v, v_lut, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaEventRecord(stop));
    const float ms = time_events(start, stop);
    ...
    const double seconds = ms / 1000.0;
    const double kq_ops = (double) n_warps * (double) cfg.iters * (double) k_tiles * (double) nfrags * 2.0 * 16.0 * 8.0 * 64.0;
    const double pv_ops = (double) reuse_factor * (double) n_warps * (double) cfg.iters * (double) k_tiles *
                          (double) nfrags * (double) groups_per_frag * 4.0 * 2.0 * 16.0 * 8.0 * 16.0;
    const double k_compact_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * (double) nfrags * 32.0 * (double) sizeof(block_nvfp4) / 1.0e9;
    const double v_compact_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * 64.0 *
                               (double) nfrags * (double) sizeof(block_nvfp4) / 1.0e9;
    const double v_lut_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * 64.0 *
    ...
    std::printf("%s: %.3f modeled-total-TOPS  %.3f useful-mtp-modeled-total-TOPS  %.3f KQ-issue-TOPS  %.3f mixedPV-issue-TOPS  %.3f GB/s-K-compact-read  %.3f GB/s-V-compact-read  %.3f GB/s-V-lut-read  lut_bytes=%zu reuse_factor=%d blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
                label,
                (kq_ops + pv_ops) / seconds / 1.0e12,
                (kq_ops + pv_ops) / seconds / 1.0e12 * useful_mtp_fraction(cfg),
                kq_ops / seconds / 1.0e12,
...
static int run_lutb_mtp4_benchmark(
    ...
    const char * label = "combined256_lutb_mtp4";
    const int active = print_occupancy(cfg, prop, label, (const void *) mixedpv_lutb_mtp4_kernel);
    if (active == 0) {
        std::printf("%s: skipped reason=kernel has zero active blocks for threads=%d\n", label, cfg.threads);
        return 0;
    }

    mixedpv_lutb_mtp4_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v, v_lut, n_blocks - 1, out, 2);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    CUDA_CHECK(cudaEventCreate(&stop));
    CUDA_CHECK(cudaEventRecord(start));
    mixedpv_lutb_mtp4_kernel<<<cfg.blocks, cfg.threads>>>(q, k, v, v_lut, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaEventRecord(stop));
    const float ms = time_events(start, stop);

    const double seconds = ms / 1000.0;
    const double kq_ops = (double) mtp_rows * (double) n_warps * (double) cfg.iters * (double) k_tiles *
                          (double) nfrags * 2.0 * 16.0 * 8.0 * 64.0;
    const double pv_ops = (double) mtp_rows * (double) n_warps * (double) cfg.iters * (double) k_tiles *
                          (double) nfrags * (double) groups_per_frag * 4.0 * 2.0 * 16.0 * 8.0 * 16.0;
    const double k_compact_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * (double) nfrags *
                                32.0 * (double) sizeof(block_nvfp4) / 1.0e9;
    const double v_compact_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * 64.0 *
                                (double) nfrags * (double) sizeof(block_nvfp4) / 1.0e9;
    const double v_lut_gb = (double) n_warps * (double) cfg.iters * (double) k_tiles * 64.0 *
                            (double) nfrags * (double) groups_per_frag * 4.0 * 2.0 * (double) sizeof(uint32_t) / 1.0e9;

    std::printf("%s: %.3f modeled-total-TOPS  %.3f useful-mtp-modeled-total-TOPS  %.3f KQ-issue-TOPS  %.3f mixedPV-issue-TOPS  %.3f GB/s-K-compact-read  %.3f GB/s-V-compact-read  %.3f GB/s-V-lut-read  lut_bytes=%zu mtp_rows=%d blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
                label,
                (kq_ops + pv_ops) / seconds / 1.0e12,
                (kq_ops + pv_ops) / seconds / 1.0e12 * useful_mtp_fraction(cfg),
                kq_ops / seconds / 1.0e12,
