...
static void print_usage(const char * exe) {
    std::printf(
        "usage: %s [options]\n"
        "\n"
        "Late-extract compact-row handoff probe for FP4-KQ + mixed-PV.\n"
        "\n"
        "options:\n"
        ...
        "  --groups N     mixed-PV groups consumed after KQ (1,2,4,8; default: 1)\n"
...
template <int GROUPS>
__global__ void lateextract_kernel(
        const block_nvfp4 * v,
        size_t              block_mask,
        float *             out,
        uint64_t            iters) {
...
#pragma unroll
            for (int group = 0; group < GROUPS; ++group) {
                const uint32_t word0 = nvfp4_group_word(blk0, group);
                const uint32_t word1 = nvfp4_group_word(blk1, group);
...
static int run_benchmark(const bench_config & cfg, const cudaDeviceProp & prop, int cc) {
    ...
    if (cc < 1200 || cc >= 1300) {
        std::printf("lateextract: skipped reason=requires Blackwell sm_120/sm_121 device\n");
        return 0;
    }

    int active_blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, lateextract_kernel<GROUPS>, cfg.threads, 0));
    ...
    std::printf("lateextract occupancy: active_blocks_per_sm=%d active_warps_per_sm=%d occupancy=%.1f%% shared=0.000 KiB\n",
    ...
    lateextract_kernel<GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, 10);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    lateextract_kernel<GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    ...
    std::printf("lateextract: %.3f measured-total-TOPS  %.3f useful-mtp-measured-total-TOPS  %.3f KQ-issue-TOPS  %.3f measured-mixedPV-TOPS  %.3f projected-fullPV-TOPS-at-same-time  %.3f GB/s-compact-read-once  pv_group_fraction=%.3f groups=%d blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
