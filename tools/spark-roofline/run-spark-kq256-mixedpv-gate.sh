...
usage() {
    cat <<'EOF'
usage: run-spark-kq256-mixedpv-gate.sh [options]

Build and run the D=256 NVFP4 KQ plus mixed-PV Spark probe.

Options:
  --build-dir DIR       CMake build directory (default: build-spark-sm121-kq256-mixedpv)
...
Environment overrides:
  ...
  NCU_KERNEL_REGEX      Kernel filter (default: regex:.*kq256_only_kernel.*)
...
This is the D=256 mixed-PV follow-up to the KQ-only Spark gate. A shipping
Spark run should use sm_121a hardware, keep Nsight enabled, and inspect the
combined256_mixed row before deciding whether D=256 moves to backend POC.
EOF
}
...
build_dir="build-spark-sm121-kq256-mixedpv"
out_dir=""
arch="121a"
device="0"
threads="128 256"
...
if [[ -z "$out_dir" ]]; then
    out_dir="/tmp/llamacpp-spark-kq256-mixedpv-$(date +%Y%m%d-%H%M%S)"
fi
...
assert_spark_log() {
    ...
    if ! grep -q '^spark_target:[[:space:]]*yes$' "$log_path"; then
        {
            echo "Spark mixed-PV gate failed: expected spark_target: yes in $log_path"
...
validate_arch() {
    if [[ "$allow_proxy" -eq 0 && "$arch" != "121a" ]]; then
        {
            echo "Spark mixed-PV gate failed: shipping Spark runs must use --arch 121a, got --arch $arch"
...
validate_cuda_toolkit() {
    ...
    if (( major < 12 || (major == 12 && minor < 8) )); then
        echo "Spark mixed-PV gate failed: CUDA >= 12.8 is required for Blackwell block-scaled FP4 MMA, got $version" | tee -a "$summary"
        exit 1
    fi

    if (( major == 13 && minor == 1 )) && [[ "${ALLOW_CUDA_13_1:-0}" != "1" ]]; then
        {
            echo "Spark mixed-PV gate failed: CUDA 13.1 is rejected by default due to the known MMQ field report."
...
validate_ncu_tool() {
    ...
    if ! command -v "$ncu_bin" >/dev/null 2>&1; then
        {
            echo "Spark mixed-PV gate failed: Nsight Compute CLI not found: $ncu_bin"
...
record_host_diagnostics() {
    local diag="$out_dir/host-diagnostics.log"
    {
        ...
        echo
        echo "## uname"
        if command -v uname >/dev/null 2>&1; then uname -a; else echo "unavailable"; fi
        echo
        echo "## os-release"
        if [[ -r /etc/os-release ]]; then sed -n '1,80p' /etc/os-release; else echo "unavailable"; fi
        echo
        echo "## cmake"
        if command -v "$cmake_bin" >/dev/null 2>&1; then "$cmake_bin" --version 2>&1 | sed -n '1,20p'; else echo "missing: $cmake_bin"; fi
        echo
        echo "## nvcc"
        if command -v nvcc >/dev/null 2>&1; then nvcc --version 2>&1 | sed -n '1,20p'; else echo "missing"; fi
        echo
        echo "## ncu"
        if command -v "$ncu_bin" >/dev/null 2>&1; then "$ncu_bin" --version 2>&1 | sed -n '1,40p'; else echo "missing: $ncu_bin"; fi
        echo
        echo "## nvidia-smi -L"
        if command -v nvidia-smi >/dev/null 2>&1; then nvidia-smi -L 2>&1; else echo "missing"; fi
        echo
...
summary="$out_dir/summary.txt"
{
    ...
    if command -v nvidia-smi >/dev/null 2>&1; then nvidia-smi -L | sed 's/^/nvidia_smi=/'; fi
    if command -v nvcc >/dev/null 2>&1; then nvcc --version | sed 's/^/nvcc=/'; fi
    if command -v "$ncu_bin" >/dev/null 2>&1; then "$ncu_bin" --version | sed 's/^/ncu=/'; fi
} | tee "$summary"
...
if [[ "$do_build" -eq 1 ]]; then
    ...
    run_logged "$out_dir/cmake-build.log" \
        "$cmake_bin" --build "$repo_root/$build_dir" --config Release --target llama-spark-kq256-mixedpv -j "$(nproc)"
fi

exe=""
for candidate in \
    "$repo_root/$build_dir/bin/llama-spark-kq256-mixedpv" \
    "$repo_root/$build_dir/bin/llama-spark-kq256-mixedpv.exe" \
    "$repo_root/$build_dir/bin/Release/llama-spark-kq256-mixedpv" \
    "$repo_root/$build_dir/bin/Release/llama-spark-kq256-mixedpv.exe" \
    "$repo_root/$build_dir/bin/RelWithDebInfo/llama-spark-kq256-mixedpv" \
    "$repo_root/$build_dir/bin/RelWithDebInfo/llama-spark-kq256-mixedpv.exe" \
    "$repo_root/$build_dir/bin/Debug/llama-spark-kq256-mixedpv" \
    "$repo_root/$build_dir/bin/Debug/llama-spark-kq256-mixedpv.exe"; do
    if [[ -x "$candidate" ]]; then
        exe="$candidate"
        break
    fi
...
if [[ -z "$exe" ]]; then
    exe="$repo_root/$build_dir/bin/llama-spark-kq256-mixedpv"
fi

if [[ "$preflight_only" -eq 1 ]]; then
    preflight_log="$out_dir/kq256-mixedpv-preflight.log"
...
for t in $(normalize_list "$threads"); do
    run_log="$out_dir/kq256-mixedpv-threads-${t}.log"
    run_logged "$run_log" \
...
if [[ "$do_ncu" -eq 1 ]]; then
    for t in $(normalize_list "$ncu_threads"); do
        report="$out_dir/ncu-kq256-mixedpv-threads-${t}"
        ncu_cmd=(
            "$ncu_bin"
            --target-processes all
            --kernel-name-base function
            --kernel-name "${NCU_KERNEL_REGEX:-regex:.*kq256_only_kernel.*}"
            --set "${NCU_SET:-full}"
            --export "$report"
            --force-overwrite
        )
        ...
        run_logged "$out_dir/ncu-kq256-mixedpv-threads-${t}.log" "${ncu_cmd[@]}"
        ...
        raw_csv="$out_dir/ncu-kq256-mixedpv-threads-${t}-raw.csv"
        import_log="$out_dir/ncu-kq256-mixedpv-threads-${t}-import.log"
        ...
        evidence_txt="$out_dir/ncu-kq256-mixedpv-threads-${t}-fp4-evidence.txt"
        evidence_json="$out_dir/ncu-kq256-mixedpv-threads-${t}-fp4-evidence.json"
        ...
        analyzer_cmd=(
            ...
            --out "$evidence_json"
            --text "$evidence_txt"
            --kernel-regex "kq256_only_kernel"
        )
        run_logged "$out_dir/ncu-kq256-mixedpv-threads-${t}-analyze.log" "${analyzer_cmd[@]}"
    done
fi

summary_csv="$out_dir/kq256-mixedpv-summary.csv"
summary_json="$out_dir/kq256-mixedpv-summary.json"
summary_txt="$out_dir/kq256-mixedpv-summary.txt"
summarizer="$repo_root/tools/spark-roofline/summarize-kq256-mixedpv-gate.py"
summarizer_cmd=(
...
run_logged "$out_dir/kq256-mixedpv-summary.log" "${summarizer_cmd[@]}"
