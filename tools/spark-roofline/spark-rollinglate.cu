...
static void print_usage(const char * exe) {
    std::printf(
        "usage: %s [options]\n"
        "\n"
        "Rolling late-extract handoff probe for full D=512 FP4-KQ + mixed-PV.\n"
        "\n"
        "options:\n"
        ...
        "  --stage-groups N  PV groups live per rolling stage (1,2,4; default: 1)\n"
...
__global__ void rollinglate_kernel(
        const block_nvfp4 * v,
        size_t              block_mask,
        float *             out,
        uint64_t            iters) {
...
static int run_benchmark(const bench_config & cfg, const cudaDeviceProp & prop, int cc) {
    ...
    if (cc < 1200 || cc >= 1300) {
        std::printf("rollinglate: skipped reason=requires Blackwell sm_120/sm_121 device\n");
        return 0;
    }

    int active_blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, rollinglate_kernel<STAGE_GROUPS>, cfg.threads, 0));
    ...
    std::printf("rollinglate occupancy: active_blocks_per_sm=%d active_warps_per_sm=%d occupancy=%.1f%% shared=0.000 KiB\n",
    ...
    rollinglate_kernel<STAGE_GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, 10);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    rollinglate_kernel<STAGE_GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    ...
    std::printf("rollinglate: %.3f measured-total-TOPS  %.3f useful-mtp-measured-total-TOPS  %.3f KQ-issue-TOPS  %.3f full-mixedPV-TOPS  %.3f GB/s-compact-read-once  stage_groups=%d full_groups=8 blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
