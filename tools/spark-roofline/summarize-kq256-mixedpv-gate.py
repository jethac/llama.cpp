...
def parse_log(path: Path) -> dict[str, object]:
    ...
    for line in text.splitlines():
        ...
        match = re.match(
            r"^combined256_lutb_reuse4:\s+([0-9.]+)\s+modeled-total-TOPS\s+([0-9.]+)\s+"
            r"useful-mtp-modeled-total-TOPS\s+([0-9.]+)\s+KQ-issue-TOPS\s+"
            r"([0-9.]+)\s+mixedPV-issue-TOPS\s+([0-9.]+)\s+GB/s-K-compact-read\s+"
            r"([0-9.]+)\s+GB/s-V-compact-read\s+([0-9.]+)\s+GB/s-V-lut-read\s+"
            r"lut_bytes=(\d+)\s+reuse_factor=(\d+)\s+blocks=(\d+)\s+warps=(\d+)\s+iters=(\d+)\s+time=([0-9.]+)\s+ms$",
            stripped,
        )
        if match:
            row["lutb_reuse4_total_tops"] = float(match.group(1))
            row["lutb_reuse4_useful_mtp_total_tops"] = float(match.group(2))
            row["lutb_reuse4_kq_issue_tops"] = float(match.group(3))
            row["lutb_reuse4_mixedpv_issue_tops"] = float(match.group(4))
            row["lutb_reuse4_k_compact_read_gbps"] = float(match.group(5))
            row["lutb_reuse4_v_compact_read_gbps"] = float(match.group(6))
            row["lutb_reuse4_v_lut_read_gbps"] = float(match.group(7))
            row["lutb_reuse4_lut_bytes"] = int(match.group(8))
            row["lutb_reuse4_reuse_factor"] = int(match.group(9))
            row["lutb_reuse4_blocks"] = int(match.group(10))
            row["lutb_reuse4_warps"] = int(match.group(11))
            row["lutb_reuse4_iters"] = int(match.group(12))
            row["lutb_reuse4_ms"] = float(match.group(13))
            continue
...
def main() -> int:
    ...
    fieldnames = [
        ...
        "lutb_lut_bytes",
        "lutb_reuse4_total_tops",
        "lutb_reuse4_useful_mtp_total_tops",
        "lutb_reuse4_kq_issue_tops",
        "lutb_reuse4_mixedpv_issue_tops",
        "lutb_reuse4_k_compact_read_gbps",
        "lutb_reuse4_v_compact_read_gbps",
    ...
    for row in rows:
        ...
        if "lutb_reuse4_total_tops" in row:
            lines.append(
                "threads={threads} lutb_reuse4_total_tops={total} "
                "useful_mtp_lutb_reuse4_total_tops={useful_total} kq_issue_tops={kq_issue} "
                "mixedpv_issue_tops={mixedpv} k_read_gbps={k_read} v_read_gbps={v_read} "
                "v_lut_read_gbps={v_lut_read} lut_bytes={lut_bytes} reuse_factor={reuse_factor}".format(
                    threads=row.get("threads", ""),
                    total=row.get("lutb_reuse4_total_tops", ""),
                    useful_total=row.get("lutb_reuse4_useful_mtp_total_tops", ""),
                    kq_issue=row.get("lutb_reuse4_kq_issue_tops", ""),
                    mixedpv=row.get("lutb_reuse4_mixedpv_issue_tops", ""),
                    k_read=row.get("lutb_reuse4_k_compact_read_gbps", ""),
                    v_read=row.get("lutb_reuse4_v_compact_read_gbps", ""),
