...
#if defined(GGML_CUDA_NVFP4_FA_P1_KX_PROB_LOGICAL_V_FP16_PV) || \
    (defined(GGML_CUDA_NVFP4_FA_NATIVE_PV_REQUANT) && defined(GGML_CUDA_NVFP4_FA_NATIVE_PV_VX))
#define GGML_CUDA_NVFP4_FA_P1_KX_SPLIT_PROB
#endif
...
#if defined(GGML_CUDA_NVFP4_FA_P1_KX_PROB_LOGICAL_V_FP16_PV)
__global__ void fattn_nvfp4_p1_split_pv_from_prob_logical_v_kernel(const fattn_nvfp4_mtp4_params params) {
#if defined(BLACKWELL_MMA_AVAILABLE)
    const int tid     = threadIdx.x;
    const int warp_id = tid / WARP_SIZE;
    const int lane    = tid & (WARP_SIZE - 1);
    ...
    if (params.ne_q_rows != 1 || q_head >= params.ne_q_heads || seq >= params.ne_seqs ||
            kv_head >= params.ne_kv_heads || kv_start >= kv_end ||
            params.split_prob == nullptr || params.split_partial == nullptr || params.v_lut == nullptr) {
        return;
    }
...
#pragma unroll
            for (int l = 0; l < tile_PV_A::ne; ++l) {
                ...
                if (row == 0 && col == 0) {
                    const float p0 =
                        params.split_prob[ggml_cuda_fattn_nvfp4_split_prob_idx(params, kv_split, seq, q_head, 0, kv_local)];
                    const float p1 = kv_local + 1 < kv_count ?
                        params.split_prob[ggml_cuda_fattn_nvfp4_split_prob_idx(params, kv_split, seq, q_head, 0, kv_local + 1)] :
                        0.0f;
                    A.x[l] = __floats2half2_rn(p0, p1);
                } else {
                    A.x[l] = __floats2half2_rn(0.0f, 0.0f);
...
#pragma unroll
        for (int l = 0; l < tile_C::ne; ++l) {
            const int row = ggml_cuda_fattn_nvfp4_pv_c_i(l, lane);
            const int col = col_base + ggml_cuda_fattn_nvfp4_pv_c_j(l, lane);
            if (row == 0 && col < params.v_head_dim) {
                const int64_t partial_idx =
                    ((((kv_split * params.ne_seqs + seq) * params.ne_q_heads + q_head) * params.ne_q_rows)
                        * params.v_head_dim + col);
                params.split_partial[partial_idx] = C.x[l];
            }
        }
...
#endif // defined(GGML_CUDA_NVFP4_FA_P1_KX_PROB_LOGICAL_V_FP16_PV)
...
__global__ void fattn_nvfp4_p1_kq8_tc_pv_split_kernel(const fattn_nvfp4_mtp4_params params) {
#if defined(BLACKWELL_MMA_AVAILABLE)
    const int tid     = threadIdx.x;
    const int warp_id = tid / WARP_SIZE;
    const int lane    = tid & (WARP_SIZE - 1);
    ...
    const int64_t seq      = (int64_t) blockIdx.y / params.kv_split_count;
    const int64_t kv_split = (int64_t) blockIdx.y - seq * params.kv_split_count;
    const int64_t kv_start = kv_split * params.kv_split_size;
    const int64_t kv_end   = min(params.ne_kv_rows, kv_start + params.kv_split_size);
    const int64_t kv_count = kv_end - kv_start;
    const int64_t kv_head  = q_head / params.gqa_ratio;
    ...
    if (params.ne_q_rows != 1 || q_head >= params.ne_q_heads || seq >= params.ne_seqs ||
            kv_head >= params.ne_kv_heads || kv_start >= kv_end ||
            params.split_partial == nullptr || params.split_meta == nullptr) {
        return;
    }
    ...
    __shared__ int      q_tile[FATTN_NVFP4_MAX_NFRAG][tile_A::J];
    __shared__ uint32_t q_tile_scale[FATTN_NVFP4_MAX_NFRAG];
    __shared__ int      kq_a_tile[tile_A::I * tile_A::J];
    __shared__ int      kq_b_tile[tile_B::I * tile_B::J];
    __shared__ uint32_t kq_a_scale[tile_A::I];
    __shared__ uint32_t kq_b_scale[tile_B::I];
    __shared__ float    score_table[FATTN_NVFP4_SPLIT_KV_ROWS];
    __shared__ float    p_table[FATTN_NVFP4_SPLIT_KV_ROWS];
...
#pragma unroll
        for (int k = 0; k < tile_A::J; ++k) {
            q_tile[frag][k] = (int) q_qs[k];
        }
        q_tile_scale[frag] = ggml_cuda_fattn_nvfp4_block_scale(q_blk);
    }

    if (tid == 0) {
        smem_kq_max = -INFINITY;
        smem_rowsum = 0.0f;
    }
    __syncthreads();
    ...
    if (warp_id == 0) {
#pragma unroll 1
        for (int64_t kv_local_base = 0; kv_local_base < kv_count; kv_local_base += tile_B::I) {
            tile_C kq_tile = {};
            ...
            for (int frag = 0; frag < nfrag; ++frag) {
                ...
                for (int i = lane; i < tile_A::I; i += WARP_SIZE) {
                    kq_a_scale[i] = q_tile_scale[frag];
                }
                for (int i = lane; i < tile_B::I; i += WARP_SIZE) {
                    kq_b_scale[i] = 0;
                }
                ...
                for (int k = lane; k < tile_A::J; k += WARP_SIZE) {
                    kq_a_tile[k] = q_tile[frag][k];
                }

                for (int kv_i = lane; kv_i < tile_B::I; kv_i += WARP_SIZE) {
                    const int64_t kv_row = kv_start + kv_local_base + kv_i;
                    if (kv_row < kv_end) {
                        const block_nvfp4 * k_ptr = params.K +
                            kv_head * params.k_stride_head +
                            seq     * params.k_stride_seq +
                            kv_row  * params.k_stride_row;
                        const block_nvfp4 k_blk = k_ptr[frag];
                        const uint32_t * k_qs = reinterpret_cast<const uint32_t *>(k_blk.qs);

#pragma unroll
                        for (int k = 0; k < tile_B::J; ++k) {
                            kq_b_tile[kv_i * tile_B::J + k] = (int) k_qs[k];
                        }
                        kq_b_scale[kv_i] = ggml_cuda_fattn_nvfp4_block_scale(k_blk);
                    }
                }
                __syncwarp();
...
#pragma unroll
            for (int kv_i = 0; kv_i < tile_B::I; ++kv_i) {
                const int64_t kv_local = kv_local_base + kv_i;
...
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    if (tile_C::get_i(l) == 0 && tile_C::get_j(l) == kv_i) {
                        local_score += kq_tile.x[l];
                    }
                }
...
#pragma unroll
                ...
                const float score = raw_score * params.scale + mask;
                const float finite_score = isinf(score) && score < 0.0f ? -INFINITY : score;
                if (lane == 0) {
                    score_table[kv_local] = finite_score;
                    smem_kq_max = fmaxf(smem_kq_max, finite_score);
                }
            }
        }
...
#pragma unroll 1
            for (int64_t kv_local = 0; kv_local < kv_count; ++kv_local) {
                const float score = score_table[kv_local];
                const float p = score - smem_kq_max >= SOFTMAX_FTZ_THRESHOLD ? expf(score - smem_kq_max) : 0.0f;
                p_table[kv_local] = p;
                rowsum += p;
            }
            smem_rowsum = rowsum;
...
#pragma unroll
                for (int l = 0; l < tile_PV_A::ne; ++l) {
                    ...
                    A.x[l] = row == 0 && col == 0 ?
                        __floats2half2_rn(
                            p_table[kv_local],
                            kv_local + 1 < kv_count ? p_table[kv_local + 1] : 0.0f) :
                        __floats2half2_rn(0.0f, 0.0f);
                }
...
#pragma unroll
    ...
    if (tid == 0) {
        const int64_t meta_idx =
            ((kv_split * params.ne_seqs + seq) * params.ne_q_heads + q_head) * params.ne_q_rows;
        params.split_meta[meta_idx] = make_float2(smem_kq_max, smem_rowsum);
    }
#else
    GGML_UNUSED(params);
#endif // defined(BLACKWELL_MMA_AVAILABLE)
}
#endif // defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && defined(GGML_CUDA_NVFP4_FA_MMA_SPLIT_KV) && defined(GGML_CUDA_NVFP4_FA_P1_KQ8_TC_PV)
...
#if defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && \
    defined(GGML_CUDA_NVFP4_FA_MMA_SPLIT_KV) && defined(GGML_CUDA_NVFP4_FA_MMA_DECOUPLED) && \
    defined(GGML_CUDA_NVFP4_FA_P1_KX_SPLIT_PROB)
__global__ void fattn_nvfp4_p1_kx_split_kq_prob_kernel(
        const fattn_nvfp4_mtp4_params params,
    ...
    for (int frag = lane; frag < nfrag; frag += WARP_SIZE) {
        const block_nvfp4 q_blk = ggml_cuda_fattn_nvfp4_quantize_q_frag(q_ptr, frag);
        const uint32_t * q_qs = reinterpret_cast<const uint32_t *>(q_blk.qs);
...
#pragma unroll 1
    for (int64_t kv_local_base = 0; kv_local_base < kv_count; kv_local_base += GGML_CUDA_NVFP4_KX_KV_ROWS) {
        float kq_score[GGML_CUDA_NVFP4_KX_KV_ROWS] = {};
        const int64_t kx_kv_tile = (kv_start + kv_local_base) / GGML_CUDA_NVFP4_KX_KV_ROWS;

#pragma unroll 1
        for (int q_frag = 0; q_frag < nfrag; ++q_frag) {
            const ggml_cuda_nvfp4_kx_tile * kx_tile =
                ggml_cuda_nvfp4_kx_tile_ptr(kx_layout, seq, kv_head, q_frag, kx_kv_tile);
            ...
            for (int i = lane; i < tile_A::I; i += WARP_SIZE) {
                kq_a_scale[i] = q_tile_scale[q_frag];
            }
            for (int i = lane; i < tile_B::I; i += WARP_SIZE) {
                kq_b_scale[i] = kx_tile->scale[i];
            }
            if (lane < tile_A::J) {
                kq_a_tile[lane] = q_tile[q_frag][lane];
            }
            __syncwarp();
...
#pragma unroll
            for (int row = 0; row < GGML_CUDA_NVFP4_KX_KV_ROWS; ++row) {
                const int64_t kv_local = kv_local_base + row;
                if (kv_local >= kv_count) {
                    break;
                }
                ...
                const int64_t kv_row = kv_start + kv_local;
                const float mask = ggml_cuda_fattn_nvfp4_mask_value(params, 0, kv_row, seq);
                float score = kq_score[row] * params.scale + mask;
                if (isinf(score) && score < 0.0f) {
                    score = -INFINITY;
                }
                score_table[kv_local] = score;
                smem_kq_max = fmaxf(smem_kq_max, score);
            }
        }
        __syncwarp();
    }
...
#pragma unroll 1
        for (int64_t kv_local = 0; kv_local < kv_count; ++kv_local) {
            const float score = score_table[kv_local];
            const float p = score - smem_kq_max >= SOFTMAX_FTZ_THRESHOLD ? expf(score - smem_kq_max) : 0.0f;
            params.split_prob[ggml_cuda_fattn_nvfp4_split_prob_idx(params, kv_split, seq, q_head, 0, kv_local)] = p;
            rowsum += p;
        }

        const int64_t meta_idx = ((kv_split * params.ne_seqs + seq) * params.ne_q_heads + q_head) * params.ne_q_rows;
        params.split_meta[meta_idx] = make_float2(smem_kq_max, rowsum);
    }
#else
    GGML_UNUSED_VARS(params, kx_layout);
#endif // defined(BLACKWELL_MMA_AVAILABLE)
}
#endif // defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && defined(GGML_CUDA_NVFP4_FA_MMA_SPLIT_KV) && defined(GGML_CUDA_NVFP4_FA_MMA_DECOUPLED) && defined(GGML_CUDA_NVFP4_FA_P1_KX_SPLIT_PROB)
...
#if defined(GGML_CUDA_NVFP4_KV_EXEC_P1_NATIVE_KQ) && \
    defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && \
    defined(GGML_CUDA_NVFP4_FA_MMA_SPLIT_KV) && defined(GGML_CUDA_NVFP4_FA_MMA_DECOUPLED) && \
    defined(GGML_CUDA_NVFP4_FA_P1_KX_PROB_LOGICAL_V_FP16_PV)
bool ggml_cuda_flash_attn_ext_nvfp4_p1_kx_logical_v_fp16_pv(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ggml_cuda_set_device(ctx.device);
    ...
    for (int64_t kv_split_base = 0; kv_split_base < params.kv_split_count; kv_split_base += params.kv_split_active_count) {
        ...
        const dim3 window_grid(1, (uint32_t) params.ne_q_heads, (uint32_t) (params.ne_seqs * params.kv_split_active_count));
        fattn_nvfp4_p1_kx_split_kq_prob_kernel<<<window_grid, kq_prob_block, 0, ctx.stream()>>>(params, kx_layout);
        fattn_nvfp4_p1_split_pv_from_prob_logical_v_kernel<<<window_grid, pv_block, 0, ctx.stream()>>>(params);
    }
#else
    const dim3 grid(1, (uint32_t) params.ne_q_heads, (uint32_t) (params.ne_seqs * params.kv_split_count));
    fattn_nvfp4_p1_kx_split_kq_prob_kernel<<<grid, kq_prob_block, 0, ctx.stream()>>>(params, kx_layout);
    fattn_nvfp4_p1_split_pv_from_prob_logical_v_kernel<<<grid, pv_block, 0, ctx.stream()>>>(params);
#endif // defined(GGML_CUDA_NVFP4_FA_ACTIVE_DECOUPLED_WINDOW_SPLITS)
...
#endif // defined(GGML_CUDA_NVFP4_KV_EXEC_P1_NATIVE_KQ) && defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && defined(GGML_CUDA_NVFP4_FA_MMA_SPLIT_KV) && defined(GGML_CUDA_NVFP4_FA_MMA_DECOUPLED) && defined(GGML_CUDA_NVFP4_FA_P1_KX_PROB_LOGICAL_V_FP16_PV)
...
bool ggml_cuda_flash_attn_ext_nvfp4_p1_kx_vx_native_pv(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    params.native_pv_vx_layout = vx_layout;
    ...
    for (int64_t kv_split_base = 0; kv_split_base < params.kv_split_count; kv_split_base += params.kv_split_active_count) {
        ...
        const dim3 window_grid(1, (uint32_t) params.ne_q_heads, (uint32_t) (params.ne_seqs * params.kv_split_active_count));
        fattn_nvfp4_p1_kx_split_kq_prob_kernel<<<window_grid, kq_prob_block, 0, ctx.stream()>>>(params, kx_layout);
        fattn_nvfp4_mtp4_split_pv_from_prob_kernel<<<window_grid, pv_block, 0, ctx.stream()>>>(params);
    }
#else
    const dim3 grid(1, (uint32_t) params.ne_q_heads, (uint32_t) (params.ne_seqs * params.kv_split_count));
    fattn_nvfp4_p1_kx_split_kq_prob_kernel<<<grid, kq_prob_block, 0, ctx.stream()>>>(params, kx_layout);
    fattn_nvfp4_mtp4_split_pv_from_prob_kernel<<<grid, pv_block, 0, ctx.stream()>>>(params);
#endif // defined(GGML_CUDA_NVFP4_FA_ACTIVE_DECOUPLED_WINDOW_SPLITS)
...
#endif // defined(GGML_CUDA_NVFP4_KV_EXEC_P1_NATIVE_KQ) && defined(GGML_CUDA_NVFP4_FA_MMA) && defined(GGML_CUDA_NVFP4_FA_MMA_MULTIWARP) && defined(GGML_CUDA_NVFP4_FA_MMA_SPLIT_KV) && defined(GGML_CUDA_NVFP4_FA_MMA_DECOUPLED) && defined(GGML_CUDA_NVFP4_FA_NATIVE_PV_REQUANT) && defined(GGML_CUDA_NVFP4_FA_NATIVE_PV_VX)
