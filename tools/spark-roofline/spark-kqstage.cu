...
static void print_usage(const char * exe) {
    std::printf(
        "usage: %s [options]\n"
        "\n"
        "KQ-boundary group-stream handoff probe for full D=512 FP4-KQ + mixed-PV.\n"
        "\n"
        "options:\n"
        ...
        "  --stage-groups N  first-stage PV groups carried across KQ (1,2,4; default: 2)\n"
...
__global__ void kqstage_kernel(
        const block_nvfp4 * v,
        size_t              block_mask,
        float *             out,
        uint64_t            iters) {
...
#pragma unroll
        for (int tile = 0; tile < 64; ++tile) {
            ...
            uint32_t pre_word0[STAGE_GROUPS];
            uint32_t pre_word1[STAGE_GROUPS];
            uint8_t pre_d0[STAGE_GROUPS];
            uint8_t pre_d1[STAGE_GROUPS];

#pragma unroll
            for (int local = 0; local < STAGE_GROUPS; ++local) {
                pre_word0[local] = nvfp4_group_word_ptr(blk0, local);
                pre_word1[local] = nvfp4_group_word_ptr(blk1, local);
                pre_d0[local] = blk0->d[local / 2];
                pre_d1[local] = blk1->d[local / 2];
            }
...
#pragma unroll
            for (int local = 0; local < STAGE_GROUPS; ++local) {
                const int group = local;
                const float d0 = ue4m3_to_fp32(pre_d0[local]);
                const float d1 = ue4m3_to_fp32(pre_d1[local]);

#pragma unroll
                for (int kk = 0; kk < 4; ++kk) {
                    const uint8_t q0 = (uint8_t) (pre_word0[local] >> (8 * kk));
                    const uint8_t q1 = (uint8_t) (pre_word1[local] >> (8 * kk));
...
#pragma nounroll
            for (int stage = STAGE_GROUPS; stage < 8; stage += STAGE_GROUPS) {
#pragma unroll
                for (int local = 0; local < STAGE_GROUPS; ++local) {
...
static int run_benchmark(const bench_config & cfg, const cudaDeviceProp & prop, int cc) {
    ...
    if (cc < 1200 || cc >= 1300) {
        std::printf("kqstage: skipped reason=requires Blackwell sm_120/sm_121 device\n");
        return 0;
    }

    int active_blocks_per_sm = 0;
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, kqstage_kernel<STAGE_GROUPS>, cfg.threads, 0));
    ...
    std::printf("kqstage occupancy: active_blocks_per_sm=%d active_warps_per_sm=%d occupancy=%.1f%% shared=0.000 KiB\n",
    ...
    kqstage_kernel<STAGE_GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, 10);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    kqstage_kernel<STAGE_GROUPS><<<cfg.blocks, cfg.threads>>>(v, n_blocks - 1, out, cfg.iters);
    CUDA_CHECK(cudaGetLastError());
    ...
    std::printf("kqstage: %.3f measured-total-TOPS  %.3f useful-mtp-measured-total-TOPS  %.3f KQ-issue-TOPS  %.3f full-mixedPV-TOPS  %.3f GB/s-compact-field-read  stage_groups=%d full_groups=8 blocks=%zu warps=%zu iters=%" PRIu64 " time=%.3f ms\n",
